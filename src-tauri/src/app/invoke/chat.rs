...
use crate::model::{
    table::Conversation, MessageItem, MessageType, StreamData, StreamEmitter, StreamError,
};
...
use crate::store::module::{ConversationManager, MessageManager};
use tauri::State;
...
pub async fn get_conversations(state: State<'_, AppState>) -> Result<Vec<Conversation>, String> {
    state.db.get_conversations().map_err(|e| e.to_string())
}
